...
public interface DatasetRequestRepository extends CrudRepository<DatasetRequest, Long> {
    ...
    @Override
    @PostFilter("(filterObject.requestOf.providedBy.username == authentication.name) || (filterObject.requestedIn.requestedBy.username == authentication.name)")
    Iterable<DatasetRequest> findAll();
}
