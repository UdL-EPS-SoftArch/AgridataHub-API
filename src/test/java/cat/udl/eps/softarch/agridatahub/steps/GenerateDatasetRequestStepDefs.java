...
public class GenerateDatasetRequestStepDefs {
    ...
    @And("I create a new DatasetRequest associate with Dataset {string} and {string} and Request with id {string} and status value {string}")
    public void iCreateANewDatasetRequestAssociateWithDatasetAndAndRequestWithIdAndStatusValue(String title, String descr, String id, String granted) throws Throwable {
        datasetRequest = new DatasetRequest();
        datasetRequest.setGranted(Boolean.parseBoolean(granted));
        datasetRequest.setRequestOf(datasetRepository.findDatasetByTitleAndDescription(title, descr));
        datasetRequest.setRequestedIn(requestRepository.findById(Long.parseLong(id)).get());
